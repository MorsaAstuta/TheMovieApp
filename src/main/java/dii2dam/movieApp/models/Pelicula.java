package dii2dam.movieApp.models;

import java.net.URL;
import java.util.Date;

public class Pelicula {

	private Integer id;
	  private String title;
	  private Integer company_id;
	  private Date release_date;
	  private String summary;
	  private URL poster_url;
	  private Date watch_date;
	  private Double rating_global;
	  private Double rating_user;
	  private String review;
	  private Integer location_id;

	  public Pelicula(Integer id, String title, Integer company_id, Date release_date, String summary, URL poster_url, Date watch_date, Double rating_global, Double rating_user, String review, Integer location_id) {
		setId(id);
		setTitle(title);
		setCompany_id(company_id);
		setRelease_date(release_date);
		setSummary(summary);
		setPoster_url(poster_url);
		setWatch_date(watch_date);
		setRating_global(rating_global);
		setRating_user(rating_user);
		setReview(review);
		setLocation_id(location_id);
	  }

	  public String getTitle() {
	    return title;
	  }
	  
	  public void setTitle(String title) {
	    this.title = title;
	  }

	  public Integer getId() {
	    return id;
	  }

	  public void setId(Integer id) {
	    this.id = id;
	  }

	  public Integer getCompany_id() {
	    return company_id;
	  }

	  public void setCompany_id(Integer company_id) {
	    this.company_id = company_id;
	  }

	  public Date getRelease_date() {
	    return release_date;
	  }

	  public void setRelease_date(Date release_date) {
	    this.release_date = release_date;
	  }

	  public String getSummary() {
	    return summary;
	  }

	  public void setSummary(String summary) {
	    this.summary = summary;
	  }

	  public URL getPoster_url() {
	    return poster_url;
	  }

	  public void setPoster_url(URL poster_url) {
	    this.poster_url = poster_url;
	  }

	  public Date getWatch_date() {
	    return watch_date;
	  }

	  public void setWatch_date(Date watch_date) {
	    this.watch_date = watch_date;
	  }

	  public Double getRating_global() {
	    return rating_global;
	  }

	  public void setRating_global(Double rating_global) {
	    this.rating_global = rating_global;
	  }

	  public Double getRating_user() {
	    return rating_user;
	  }

	  public void setRating_user(Double rating_user) {
	    this.rating_user = rating_user;
	  }

	  public String getReview() {
	    return review;
	  }

	  public void setReview(String review) {
	    this.review = review;
	  }

	  public Integer getLocation_id() {
	    return location_id;
	  }

	  public void setLocation_id(Integer location_id) {
	    this.location_id = location_id;
	  }
	
}
